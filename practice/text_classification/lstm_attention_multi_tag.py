#!/usr/bin/env python2
#-*- coding: utf-8 -*-
#@Filename : lstm_attention_multi_tag
#@Date : 2018-08-21-17-51
#@AUTHOR : bai
...
class Model(object):
    def __init__(self,
        ...
        self.sentence_placeholder = tf.placeholder(
            ...
            name="sentence")
        self.label_target = tf.placeholder(tf.int32, shape=[None])
    ...
    def loss(self, inputs, reuse=None):
        ...
        top5 = tf.nn.in_top_k(preds, target, 5)
        t3 = tf.reduce_sum(tf.cast(top3, tf.int32))
        t5 = tf.reduce_sum(tf.cast(top5, tf.int32))
