#!/usr/bin/env python2
#-*- coding: utf-8 -*-
#@Filename : train_lstm_att_multi_tag
...
import lstm_attention_multi_tag as direction_model
...
tf.app.flags.DEFINE_string('test_data_path', "/Users/higgs/codebase/all-about-nlp/practice/text_classification/test_yy.tfrecord",
                           'Test data path')
tf.app.flags.DEFINE_string('log_dir', "logs_sa", 'The log  dir')
...
def test_eval(sess, loss, tt3, tt5, testDatas, batchSize, sentence_h,
              target_h, targets_h):
    ...
    for i in range(numBatch):
        ...
        feed_dict = {sentence_h: sentences,
                     target_h: target,
                     targets_h: targets}
        lossv, t3v, t5v = sess.run([loss, tt3, tt5], feed_dict)
        totalLoss += lossv
        t3 += t3v
        t5 += t5v
    return totalLoss / numBatch, float(t3) / totalLen, float(t5) / totalLen
...
def main(unused_argv):
    ...
    with graph.as_default():
        ...
        loss, _, _ = model.loss(batch_inputs)
        testLoss, tt3, tt5= model.test_loss()
            ...
            for step in range(training_steps):
                ...
                try:
                    ...
                    if (step + 1) % 200 == 0 or step == 0:
                        tloss, acc3, acc5 = test_eval(
                            sess, testLoss, tt3, tt5, testDatas,
                            FLAGS.test_batch_size, model.sentence_placeholder,
                            model.label_target, model.targets)

                        print("test loss:%.3f, top3 acc %.3f, top5 acc %.3f" %
                              (tloss, acc3, acc5))
